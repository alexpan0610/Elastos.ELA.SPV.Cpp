	...
	namespace ElaWallet {
		...
		bool NotifyQueue::Upsert(const RecordPtr record) {
			return DoTransaction([&record, this]() {
				...
				sql = "REPLACE INTO " + NOTIFY_QUEUE_TABLE + "(" + NOTIFY_QUEUE_COLUMN_TX_HASH + "," +
					  NOTIFY_QUEUE_COLUMN_HEIGHT + "," + NOTIFY_QUEUE_COLUMN_LAST_NOTIFY_TIME + ") VALUES(?,?,?);";
		...
		NotifyQueue::Records NotifyQueue::GetAllConfirmed(uint32_t current) {
			...
			DoTransaction([&rows, current, this]() {
				...
				sql = "SELECT " + NOTIFY_QUEUE_COLUMN_TX_HASH + "," + NOTIFY_QUEUE_COLUMN_HEIGHT + "," +
					  NOTIFY_QUEUE_COLUMN_LAST_NOTIFY_TIME + " FROM " + NOTIFY_QUEUE_TABLE +
		...
		bool NotifyQueue::Delete(const uint256 &tx_hash) {
			return DoTransaction([&tx_hash, this]() {
				...
				sql = "DELETE FROM " + NOTIFY_QUEUE_TABLE + " WHERE " +
					  NOTIFY_QUEUE_COLUMN_TX_HASH + " = '" + tx_hash.GetHex() + "';";
