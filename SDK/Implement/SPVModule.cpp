...
namespace Elastos {
	namespace ElaWallet {
		...
		void SPVModule::onTxAdded(const TransactionPtr &tx) {
			...
			Log::debug("onTxAdded(hash='{}', height={})", tx->GetHash().ToString(), tx->GetBlockHeight());
			_notify_queue.Upsert(NotifyQueue::RecordPtr(new NotifyQueue::Record(
				uint256(tx->GetHash().GetHex()), tx->GetBlockHeight())));
		}

		void SPVModule::onTxUpdated(const std::vector<uint256> &hashes,
									uint32_t block_height, time_t timestamp) {
			SpvService::onTxUpdated(hashes, block_height, timestamp); // Call parent.

			for (auto hash : hashes) {
				Log::debug("onTxUpdated(hash='{}', height={})", hash.ToString(), block_height);
				_notify_queue.Upsert(
					NotifyQueue::RecordPtr(new NotifyQueue::Record(hash, block_height)));
			}
		}
