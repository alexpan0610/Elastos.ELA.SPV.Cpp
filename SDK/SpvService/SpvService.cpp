...
SpvService::SpvService(const std::string &walletID,
    ...
    : CoreSpvService(pluginTypes, chainParams),
      _executor(BACKGROUND_THREAD_COUNT), _databaseManager(dbPath) {
  init(walletID, subAccount, earliestPeerTime, reconnectSeconds);
}

SpvService::~SpvService() { _executor.StopThread(); }
...
void SpvService::ExecutorStop() { _executor.StopThread(); }
...
void SpvService::DatabaseFlush() { _databaseManager.flush(); }
...
void SpvService::onCoinBaseTxAdded(const UTXOPtr &cb) {
  ...
  std::for_each(
      _walletListeners.begin(), _walletListeners.end(),
      [&cb](Wallet::Listener *listener) { listener->onCoinBaseTxAdded(cb); });
}
...
void SpvService::onCoinBaseTxUpdated(const std::vector<uint256> &hashes,
...
void SpvService::onCoinBaseTxDeleted(const uint256 &hash, bool notifyUser,
...
void SpvService::onTxAdded(const TransactionPtr &tx) {
  ...
  std::for_each(_walletListeners.begin(), _walletListeners.end(),
                [&tx](Wallet::Listener *listener) { listener->onTxAdded(tx); });
}

void SpvService::onTxUpdated(const std::vector<uint256> &hashes,
                             uint32_t blockHeight, time_t timestamp) {
...
void SpvService::onTxDeleted(const uint256 &hash, bool notifyUser,
                             bool recommendRescan) {
...
void SpvService::onTxUpdatedAll(const std::vector<TransactionPtr> &txns) {
  ...
  std::for_each(
      _walletListeners.begin(), _walletListeners.end(),
      [&txns](Wallet::Listener *listener) { listener->onTxUpdatedAll(txns); });
}

void SpvService::onAssetRegistered(const AssetPtr &asset, uint64_t amount,
...
void SpvService::syncStarted() {
  std::for_each(
      _peerManagerListeners.begin(), _peerManagerListeners.end(),
      [](PeerManager::Listener *listener) { listener->syncStarted(); });
}
...
void SpvService::syncProgress(uint32_t currentHeight, uint32_t estimatedHeight,
  ...
  std::for_each(_peerManagerListeners.begin(), _peerManagerListeners.end(),
                [&currentHeight, &estimatedHeight,
                 &lastBlockTime](PeerManager::Listener *listener) {
...
void SpvService::txStatusUpdate() {
  std::for_each(
      _peerManagerListeners.begin(), _peerManagerListeners.end(),
      [](PeerManager::Listener *listener) { listener->txStatusUpdate(); });
}

void SpvService::saveBlocks(bool replace,
  ...
  if (blocks.size() == 1) {
    Log::debug("{} checkpoint ====> ({},  \"{}\", {}, {}),",
               _peerManager->GetID(), blocks[0]->GetHeight(),
               blocks[0]->GetHash().GetHex(), blocks[0]->GetTimestamp(),
               blocks[0]->GetTarget());
  }
#endif
...
void SpvService::txPublished(const std::string &hash,
...
std::vector<PeerInfo> SpvService::loadPeers() {
  ...
  for (size_t i = 0; i < peersEntity.size(); ++i) {
    peers.push_back(PeerInfo(peersEntity[i].address, peersEntity[i].port,
                             peersEntity[i].timeStamp));
  }
...
const CoreSpvService::PeerManagerListenerPtr &
SpvService::createPeerManagerListener() {
...
const CoreSpvService::WalletListenerPtr &SpvService::createWalletListener() {
  if (_walletListener == nullptr) {
    _walletListener =
        WalletListenerPtr(new WrappedExecutorWalletListener(this, &_executor));
  }
