...
namespace Elastos {
	namespace ElaWallet {
		SideAccount::SideAccount(const uint256 &genesis_hash)
			: SubAccount(AccountPtr(new FakeParent()), 0) {
		...
		bool SideAccount::IsProducerDepositAddress(const Address &) const {
			return false;
		}

		bool SideAccount::IsOwnerAddress(const Address &address) const { return false; }

		bool SideAccount::IsCRDepositAddress(const Address &address) const {
			return false;
		}

		void SideAccount::AddUsedAddrs(const Address &) {}

		size_t SideAccount::GetAllAddresses(std::vector<Address> &addr, uint32_t,
											size_t, bool) const {
			addr.clear();
		...
		bool SideAccount::FindKey(Key &, const bytes_t &, const std::string &) {
			return false;
		}

		bool SideAccount::ContainsAddress(const Address &address) const {
			return side_address == address;
		}

		void SideAccount::ClearUsedAddresses() {}

		bool SideAccount::GetCodeAndPath(const Address &, bytes_t &,
										 std::string &) const {
			return false;
		}

		size_t SideAccount::InternalChainIndex(const TransactionPtr &tx) const {
			return -1;
		}

		size_t SideAccount::ExternalChainIndex(const TransactionPtr &tx) const {
			return -1;
		}
